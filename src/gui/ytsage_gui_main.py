...
from src.core.ytsage_utils import check_ffmpeg, load_saved_path, save_path, should_check_for_auto_update
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                try:
                    ...
                    self.signals.update_status.emit("Error: Could not extract video information. Please check your link.")
                    self.signals.playlist_info_label_visible.emit(False)
                    self.signals.playlist_select_btn_visible.emit(False)
                    return
                ...
                try:
                    ...
                    if basic_info.get("_type") == "playlist":
                        ...
                        try:
                            ...
                            self.signals.update_status.emit(f"Error: Failed to extract info for the first playlist video: {e}")
                            self.signals.playlist_info_label_visible.emit(False)
                            self.signals.playlist_select_btn_visible.emit(False)
                            return
                    ...
                    self.signals.update_status.emit(f"Error: Failed to extract video details: {str(e)}")
                    self.signals.playlist_info_label_visible.emit(False)
                    self.signals.playlist_select_btn_visible.emit(False)
                    return
